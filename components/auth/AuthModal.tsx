  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0 }}
      >
        <div className="absolute inset-0 bg-black/70" onClick={() => { setShowForgot(false); setForgotSent(false); closeAuthModal(); }} />
        ...
        >
          <h2 className="text-xl font-bold text-green-100 text-center mb-6">
            {forgotSent ? "请查看邮箱" : showForgot ? "重置密码" : authModalMode === "signup" ? "注册保存你的报告" : "登录查看历史报告"}
          </h2>

          {forgotSent ? (
            <div className="text-center space-y-4">
              <p className="text-white/60 text-sm">
                如果邮箱已注册，我们会发送一封密码重置邮件。
              </p>
              <p className="text-white/30 text-xs">请检查收件箱（和垃圾邮件箱）</p>
              <button
